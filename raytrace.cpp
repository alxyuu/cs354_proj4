...
void initScene () {
 ...
 //s3->m = makeMaterial(0.15,0.1,1.0,0.5,0.7,0.3,0.3);
  c1 = makeCube(0.2,-0.1,-1.5,0.09, makeMaterial(0.15,0.1,1.0,0.5,0.7,0.3,0.3) );
  p1 = makePlane(0,0,-4,-3.5,2,3,1);
...
void drawScene () {
  ...
  for (i=0; i<width; i++) {
    ...
    for (j=0; j<height; j++) {
      ...
      worldPix.y = (j-(height/2))*imageWidth/width - RAYSHIFT;
      /* x position = (pixel width/middle) scaled to world coords */ 
      worldPix.x = (i-(width/2))*imageWidth/width - RAYSHIFT;
      ...
      /* write the pixel! */
      drawPixel(i,j,f_c.r,f_c.g,f_c.b);
    }
  }
}
...
void firstHit(ray* r, point* p, vector* n, material* *m) {
  ...
  hit3 = rayCubeIntersect(r2,c1,&t3,n) /*&& t3 > EPSILON*/;
